...
def create_additional_features(input_merged, plot=False):
    ...
    wav = wav[["ekg", "ppg", "prox"] + project_configs_simon.nibp_column_names + ["art"]]
...
def filter_valid_waveforms(patient_id):
    ...
    if patient_id in train_ids or patient_id in lock_file_ids:
        ...
        np.save(os.path.join(project_configs_simon.window_save_dir, "npy/{}_preprocessed.npy".format(patient_id)), wav)
...
if __name__ == "__main__":
    ...
    if multithread:
        ...
        results = pool.map(filter_valid_waveforms, [p for p in patient_ids])
        pool.close()
    else:
